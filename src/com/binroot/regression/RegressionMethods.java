package com.binroot.regression;
  ...
  public static  double[] exponential(double[] x, double[] y) throws NotEnoughValues {
    ...
    if (X.size()==Y.size()) {
      n = X.size();
        ...
        System.out.println("REGMATH1: "+( Y.log().sum() ) );
        System.out.println("REGMATH2: "+( X.pow(2).sum() ) );


        a = (( Y.log().sum() * X.pow(2).sum() ) - ( X.sum() * Y.log().multiply(X).sum() )) / ( n*X.pow(2).sum() - Math.pow(X.sum(),2) );
      System.out.println("REGMATHa: "+a);
...
/*
  ...
  // x enthalt bereits die werte der regressionskurve
